...
export function CampaignComposer() {
  const [result, setResult] = useState(
    "Select a campaign goal and generate a launch brief.",
  );
  ...
  async function generateBrief(values: FormValues) {
    ...
    trackEvent("campaign_brief_generated", {
    ...
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <Card className="glass">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-neon-400" /> Campaign composer
          </CardTitle>
          <CardDescription>
            Generate a launch brief with AI and wire it into the release
            workflow.
          </CardDescription>
        </CardHeader>
        <CardBody className="space-y-4">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" variant="accent" className="w-full">
              <CalendarDays className="h-4 w-4" /> Generate launch brief
            </Button>
          </form>
      ...
      <Card className="glass">
        <CardHeader>
          ...
          <CardDescription>
            Use this draft as a launch brief, email source doc, or SEO plan.
          </CardDescription>
        </CardHeader>
        <CardBody>
